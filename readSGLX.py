...
##KS Added! read offsets.txt and return as dataframe 
def readOffsetsTxt(file_path):
    rows = {}
    with open(file_path, "r") as f:
        for line in f:
            ...
            rows[key] = [float(x) for x in values.split("\t") if x != ""]
    
    df = pd.DataFrame(rows)
    return df 

def makeFinalOffsetDF(pass1df,pass2df):

    colNames = pass2df.columns

    if 'smp_imap100' in colNames: ##case where shanks are split just keep one shank the values are same for the whole probe 
        keepcols = ['smp_imap100','smp_nidq','sec_imap100','sec_nidq']
        pass2keep = pass2df.loc[:,keepcols]
        renamecols =  ['smp_imap0','smp_nidq','sec_imap0','sec_nidq']
        pass2keep.columns = renamecols

    elif 'smp_imap0' in colNames:
        keepcols = ['smp_imap0','smp_nidq','sec_imap0','sec_nidq']
        pass2keep = pass2df.loc[:,keepcols]

    gStarts = np.where(pass1df.sec_imap0 == 0)[0]
    
    assert (len(gStarts)) == len(pass2df), 'supercat offset size doesnt match pass 1 indices w 0'
    
    fixedlists = []
    for n,g0 in enumerate(gStarts):
        if n < len(gStarts) - 1:
            g1 = gStarts[n+1]
        else:
            g1 = len(pass1df)

        sub  = pass1df.iloc[g0:g1,:].copy()
        updatecols = pass2keep.columns
        sub.loc[:, updatecols] = sub.loc[:, updatecols].to_numpy() + pass2keep.iloc[n, :].to_numpy()
        fixedlists.append(sub)

    fixedDf = pd.concat(fixedlists)
    
    assert len(fixedDf) == len(pass1df), 'post fixed DF size doesnt match original size '

    realSecNI = np.diff(fixedDf.sec_nidq)
    realSecIM = np.diff(fixedDf.sec_imap0)
    
    assert np.all(realSecNI>0), ' real Sec NI is not strictly increasing'
    assert np.all(realSecIM>0), 'real Sec IM is not strictly increasing '


    fixedDf['realFileSecNI'] = np.append(realSecNI,np.nan)
    fixedDf['realFileSecIM'] = np.append(realSecIM,np.nan)
